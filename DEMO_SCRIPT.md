...
3. Know your numbers:
   ...
   - Demo Mode plays **18 scripted incidents** across all 3 attack types (origin hijack, sub-prefix hijack, route leak), 9 sectors, and every severity tier
   - Every incident, action, AI decision, and chat message is persisted to a **live Supabase Postgres backend** — not just the browser

---

## New: Backend Persistence + Change Log (judge-facing proof of state)

SHYEN now writes every meaningful event to a hosted Postgres database (Supabase), not just localStorage. This is worth 20 seconds on stage:

**[Open the 🕘 CHANGE LOG panel]**

> "Every incident, every action, every AI decision — this isn't just a UI animation, it's a structured audit trail. Watch the counter next to CHANGE LOG climb as Demo Mode runs — that's real, persisted state, and it's simultaneously being written to a Postgres database we control, not just this browser tab."

**[Optional: switch to the Supabase dashboard in another tab, show the `incidents` / `change_log` table filling live]**

> "This is the same data, live in our database. If this were deployed for CERT-In, this is the record they'd audit."
...
**"Where does the data actually live? Is this just in the browser tab?"**
> "No — every incident, action, AI decision, and chat message is written to a hosted Postgres database via Supabase, in addition to the in-browser state. Refresh the page, or open it on another machine, and the change log and incident history are still there. That's what makes this an actual system of record, not a demo animation."
...
- **Supabase/backend sync fails or is slow:** By design this never blocks the UI — incident detection, actions, and the dashboard all work identically whether the backend write succeeds or not (check the browser console for a `[supabase]` warning, but say nothing on stage — it's silent and non-fatal).
...
- [ ] Supabase env vars set (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`) — confirm rows appear in the Supabase Table Editor after a test action
