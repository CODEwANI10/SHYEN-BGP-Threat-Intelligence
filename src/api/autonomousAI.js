/**
 * Autonomous AI Decision Engine — v5 (Real models, real data)
 *
 * MODEL UPGRADES:
 *  QUEUE 1  →  VITE_GROQ_API_KEY    →  openai/gpt-oss-120b  (was: llama-3.3-70b-versatile, deprecated 08/16/26)
 *              High-accuracy JSON autonomous decisions
 *
 *  QUEUE 2  →  VITE_GROQ_API_KEY_2  →  openai/gpt-oss-20b   (was: llama-3.1-8b-instant, deprecated 08/16/26)
...
// ── Models ────────────────────────────────────────────────────────────────
// Groq deprecated llama-3.3-70b-versatile and llama-3.1-8b-instant (shutdown 08/16/26).
// Official replacements per console.groq.com/docs/deprecations.
// Configurable via VITE_GROQ_MODEL_ACCURATE / VITE_GROQ_MODEL_FAST so the
// model can be swapped without touching code — falls back to the current
// defaults when those env vars aren't set.
const GROQ_DECISION_MODEL = import.meta.env.VITE_GROQ_MODEL_ACCURATE || 'openai/gpt-oss-120b' // best accuracy for JSON (replaces llama-3.3-70b-versatile)
const GROQ_TEXT_MODEL     = import.meta.env.VITE_GROQ_MODEL_FAST     || 'openai/gpt-oss-20b'   // fast, high RPM (replaces llama-3.1-8b-instant)
  ...
  return queue1.schedule(async () => {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature,
        reasoning_effort: 'low', // gpt-oss-120b is a reasoning model; keep it fast for time-sensitive BGP decisions
        messages: [
  ...
  return q.schedule(async () => {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: maxTokens,
        temperature,
        reasoning_effort: 'low', // gpt-oss-20b is a reasoning model; keep responses fast
        messages: [
          { role: 'system', content: systemPrompt },
          { role: 'user',   content: userContent },
        ],
