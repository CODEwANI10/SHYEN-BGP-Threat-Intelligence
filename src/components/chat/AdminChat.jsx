...
import { chatWithGroq } from '../../api/groqActions.js'
...
export default function AdminChat() {
  ...
  // grokApiKey removed — Groq key is now read from VITE_GROQ_API_KEY inside the API file.
  ...
  const send = async (text) => {
    ...
    try {
      ...
      const reply = await chatWithGroq(all, SYSTEM_CTX(getIncidents(), getActivityLog()))
    ...
    <div style={{ display:'flex',flexDirection:'column',height:'100%' }}>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        ...
        <div style={{ display:'flex',gap:8,alignItems:'center' }}>
          <span style={{ fontFamily:'monospace',fontSize:8,color:'rgba(70,200,255,0.9)' }}>Powered by Groq</span>
